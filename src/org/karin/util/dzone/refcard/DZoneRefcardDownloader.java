...
public class DZoneRefcardDownloader {
	...
	public static class Refcard implements Comparable<Refcard>, Serializable {
		...
		public String toString() {
			return String.format("no:%d id:%d url:[%s] filename:[%s] title:[%s]", 
					number, id, url, filename, title);
		}
	...
	private void go() throws IOException, ClassNotFoundException {
		...
		if (cntnew + cntnotexist == 0) {
			...
			for (Refcard refcard : refcardsNeedDownload) {
				...
				log(String.format("Downloading [%d] [%s]", refcard.number, refcard.title));
				...
				try (Action action = new Action(pdfurl)) {
					...
					if (rc == 200) {
						...
						pdfurl = URLDecoder.decode(action.getHeader("Location"), UTF8); 
						refcard.url = pdfurl;
						...
						if (pdffile.exists() && pdffile.length() > 0) {
							...
							try (Action a = new Action(pdfurl)) {
								try (OutputStream fo = new FileOutputStream(fn)) {
									byte[] pdfdata = a.getBinary();
									if (pdfdata == null) {
										log(a.getTitle());
										try (Writer w = new FileWriter(fn.replace(".pdf", ".htm"))) {
											w.write(a.getHtml());
										}
									} else {
